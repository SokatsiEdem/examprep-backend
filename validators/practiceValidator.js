...
exports.validateStartPractice = [
  ...
  body('year')
    .optional()
    .custom((val) => val === 'random' || (!isNaN(val) && Number(val) > 1900)),
];

exports.validateSubmitPractice = [
  param('id').isUUID().withMessage('Invalid practice session ID'),
  body('answers').isArray().withMessage('Answers must be an array'),
  body('answers.*.questionId').isUUID().withMessage('Invalid question ID'),
];

exports.validateQuestionQuery = [
